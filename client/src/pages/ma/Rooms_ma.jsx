        ...
        <div className='guests'>
        {
          rooms.map((room) => {
            return <Card style={{width:'16rem',margin:10,background:`${room.color}`}} key={room._id}>
              ...
              <Button  onClick={()=>{navigate(`/ma/room/${room._id}`);handleClick()}} style={{margin:10}} >Update</Button>
              {room.book==="Vacant"&& <Button variant='danger' onClick={()=>{navigate(`/ma/addguest/${room._id}`);handleClick()}}  style={{marginLeft:10,marginRight:10,marginBottom:10}} >Book a room</Button>}
               {room.book==="Booked"&&room.guests.length>=1&&<Button variant='success' href={`/ma/generateBill/${room.guests[room.guests.length-1]._id}`}style={{marginLeft:10,marginRight:10,marginBottom:10}} >Generate Bill</Button>}
              </Card>
